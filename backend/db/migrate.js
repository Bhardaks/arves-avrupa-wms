...
async function runMigration(externalDb = null) {
  ...
  try {
    ...
    // Fix service_requests table - ensure created_at column exists
    await ensureColumn(db, 'service_requests', 'created_at', 'created_at DATETIME DEFAULT CURRENT_TIMESTAMP');
    await ensureColumn(db, 'ssh_inventory', 'notes', 'notes TEXT');
    ...
    console.log('🔧 Creating critical missing tables...');
    ...
    // Create role_permissions table - THE CRITICAL MISSING TABLE!
    await runSql(`
    ...
    console.log('✅ Critical missing tables created');
