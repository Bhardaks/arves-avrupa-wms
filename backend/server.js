...
const netsisRestAPI = require('./services/netsis-rest-api');
...
// Railway database reset check
const RAILWAY_DB_RESET = process.env.RAILWAY_DB_RESET === 'true';
console.log(`🗑️ Railway DB Reset: ${RAILWAY_DB_RESET ? 'YES - WILL DELETE DATABASE' : 'NO'}`);

// Railway data backup/restore variables
const RAILWAY_DATA_BACKUP = process.env.RAILWAY_DATA_BACKUP === 'true';
const RAILWAY_DATA_RESTORE = process.env.RAILWAY_DATA_RESTORE === 'true';
const RAILWAY_BACKUP_SOURCE = process.env.RAILWAY_BACKUP_SOURCE || null;
console.log(`💾 Railway Data Backup: ${RAILWAY_DATA_BACKUP ? 'YES - WILL CREATE BACKUP' : 'NO'}`);
console.log(`🔄 Railway Data Restore: ${RAILWAY_DATA_RESTORE ? 'YES - WILL RESTORE FROM BACKUP' : 'NO'}`);
if (RAILWAY_BACKUP_SOURCE) console.log(`📂 Backup Source: ${RAILWAY_BACKUP_SOURCE}`);

// Database connection (now handles Railway persistent volume)
const db = require('./db/db');

// Initialize backup system for local development
let dbBackup;
if (!isRailway) {
...
// Disable foreign keys globally (Railway compatibility)
...
// Handle Railway data backup/restore operations
if (isRailway && (RAILWAY_DATA_BACKUP || RAILWAY_DATA_RESTORE)) {
  const fs = require('fs');
  const path = require('path');
  
  if (RAILWAY_DATA_BACKUP) {
    console.log('💾 Creating Railway data backup...');
    try {
      // Create backup using SQLite .backup command
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const backupPath = `/data/railway_backup_${timestamp}.db`;
      
      db.serialize(() => {
        db.run('VACUUM');
        db.backup(backupPath);
        console.log(`✅ Backup created: ${backupPath}`);
        console.log(`📋 To restore, set RAILWAY_DATA_RESTORE=true and RAILWAY_BACKUP_SOURCE=${path.basename(backupPath)}`);
      });
    } catch (backupErr) {
  ...
  if (RAILWAY_DATA_RESTORE && RAILWAY_BACKUP_SOURCE) {
    console.log(`🔄 Restoring Railway data from: ${RAILWAY_BACKUP_SOURCE}`);
    try {
      const restorePath = path.join('/data', RAILWAY_BACKUP_SOURCE);
      if (fs.existsSync(restorePath)) {
        // Close current connection and restore backup
        db.close(() => {
          fs.copyFileSync(restorePath, '/data/wms.db');
          console.log(`✅ Data restored from: ${RAILWAY_BACKUP_SOURCE}`);
          console.log('🔄 Restarting with restored data...');
          process.exit(0); // Let Railway restart the service
        });
      } else {
        console.error(`❌ Backup file not found: ${restorePath}`);
        console.log('📂 Available backups:', fs.readdirSync('/data').filter(f => f.includes('backup')));
      }
    } catch (restoreErr) {
...
async function initDatabase() {
  try {
    ...
    // Railway database reset functionality
    if (isRailway && RAILWAY_DB_RESET) {
      console.log('🗑️ RAILWAY DB RESET ACTIVATED - Deleting existing database...');
      try {
        const fs = require('fs');
        const dbPath = '/data/wms.db';
        if (fs.existsSync(dbPath)) {
          fs.unlinkSync(dbPath);
          console.log('✅ Railway: Existing database deleted successfully');
        } else {
          console.log('ℹ️ Railway: No existing database found to delete');
        }
      } catch (resetError) {
        console.error('❌ Railway: Database reset failed:', resetError.message);
      }
    }
    ...
    if (isRailway) {
      ...
      // CRITICAL: Create role_permissions table for Railway
      await run(`
      ...
      `);
      console.log('🚂 Railway: role_permissions table created/verified');
      
      // Insert essential admin permissions for Railway
      await run(`
      ...
      `);
      console.log('🚂 Railway: Essential admin permissions inserted');
...
app.get('/api/roles', requireRole(['admin']), async (req, res) => {
  try {
    ...
    // Add default role information
    const defaultRoles = [
      { role: 'admin', displayName: 'Yönetici', description: 'Tüm sistem yetkilerine sahip' },
      { role: 'operator', displayName: 'Operatör', description: 'Ürün ve sipariş yönetimi yetkilerine sahip' },
      { role: 'service', displayName: 'Servis Teknisyeni', description: 'SSH servis hub yetkilerine sahip' }
    ];
    ...
    roles.forEach(r => {
      if (!defaultRoles.find(dr => dr.role === r.role)) {
        allRoles.push({
          role: r.role,
          displayName: r.role.charAt(0).toUpperCase() + r.role.slice(1),
...
app.post('/api/roles', requireRole(['admin']), async (req, res) => {
  try {
    const { roleName, displayName, description, defaultPermissions = [] } = req.body;
    
    if (!roleName) {
      return res.status(400).json({
        success: false,
        message: 'Rol adı gerekli'
      });
    }
    ...
    if (!/^[a-z][a-z0-9_-]*$/.test(roleName)) {
      return res.status(400).json({
        success: false,
        message: 'Rol adı küçük harfle başlamalı ve sadece küçük harf, sayı, tire ve alt çizgi içermelidir'
      });
    }
    ...
    const defaultPerms = defaultPermissions.length > 0 ? defaultPermissions : ['products.view', 'orders.view'];
    ...
    res.json({
      success: true,
      message: `${displayName || roleName} rolü başarıyla oluşturuldu`,
      role: {
        role: roleName,
        displayName: displayName || roleName.charAt(0).toUpperCase() + roleName.slice(1),
        description: description || 'Özel rol',
...
// Delete role
app.delete('/api/roles/:roleName', requireRole(['admin']), async (req, res) => {
  try {
    const { roleName } = req.params;
    
    // Protect default roles
    const defaultRoles = ['admin', 'operator', 'service'];
    ...
    if (usersWithRole.count > 0) {
      return res.status(400).json({
        success: false,
        message: `Bu role sahip ${usersWithRole.count} kullanıcı var. Önce kullanıcıların rollerini değiştirin.`
      });
    }
    ...
    res.json({
      success: true,
      message: `${roleName} rolü başarıyla silindi`
    });
  } catch (error) {
...
app.post('/api/netsis/sync/products', async (req, res) => {
  try {
    ...
    // PK package matching işlemi kapatıldı
    console.log('🚫 PK paket eşleştirme işlemi kapatılmış durumda');
...
app.get('/api/orders/:id', async (req, res) => {
  try {
    ...
    // Get order items with package information for delivery verification
    ...
    // Get package information for each item
    for (let item of items) {
      if (item.product_id) {
        const packages = await all(`
          SELECT pp.id, pp.package_number, pp.package_name, pp.barcode, pp.quantity,
                 pp.package_content
...
app.get('/api/shelf-assignments', async (req, res) => {
  try {
    // Önce shelf_packages tablosunun var olup olmadığını kontrol et
    ...
    if (!hasShelfPackages) {
      console.log('⚠️ shelf_packages tablosu yok, boş array döndürülüyor');
      return res.json({ assignments: [], count: 0 });
    }
    ...
    const query = `
      SELECT 
        ...
        pp.package_name,
        pp.barcode,
        pp.contents as package_content,
        pp.package_number,
        pp.product_id,
...
// Get orders ready for delivery (FULFILLED status)
app.get('/api/deliveries/orders', requireAuth, (req, res) => {
  console.log('📦 Fetching orders ready for delivery...');
  
  // SQL query to get FULFILLED orders with their items and delivery status
  const query = `
    SELECT o.*,
           COALESCE(d.delivery_status, NULL) as delivery_status,
           COALESCE(d.delivered_at, NULL) as delivered_at,
  ...
  db.all(query, [], (err, orders) => {
    ...
    if (!orders || orders.length === 0) {
      console.log('📦 No orders ready for delivery found');
      return res.json([]);
    }
    
    console.log(`📦 Found ${orders.length} orders ready for delivery`);
    orders.forEach(order => {
      console.log(`  Order ${order.id}: fulfillment=${order.fulfillment_status}, delivery=${order.delivery_status}`);
    });
    
    // Get items for each order
    const orderIds = orders.map(o => o.id);
    const itemsQuery = `
      SELECT oi.*, p.name as product_name, p.color as product_color,
             p.main_barcode, pp.barcode as package_barcode
      FROM order_items oi
    ...
    db.all(itemsQuery, orderIds, (itemsErr, items) => {
      if (itemsErr) {
...
app.post('/api/deliveries', requireAuth, (req, res) => {
  ...
  console.log(`🚚 Creating delivery record for order ${order_id}`);
  ...
  db.get(
    ...
    (err, existing) => {
      if (err) {
        ...
        return res.status(500).json({ error: 'Failed to check existing delivery' });
      }
      
      if (existing) {
        console.log(`🚚 Delivery already exists with ID ${existing.id}`);
        return res.json({ success: true, id: existing.id });
      }
      
      // Get order info
      db.get(
        'SELECT order_number, customer_name FROM orders WHERE id = ?',
        [order_id],
        (orderErr, order) => {
          if (orderErr) {
            console.error('❌ Order fetch error:', orderErr);
            return res.status(500).json({ error: 'Failed to fetch order info' });
          }
          ...
          const query = `
            INSERT INTO deliveries (order_id, order_number, customer_name, delivery_status)
            VALUES (?, ?, ?, 'pending')
          `;
          
          db.run(query, [order_id, order.order_number, order.customer_name], function(insertErr) {
            if (insertErr) {
              console.error('❌ Delivery creation error:', insertErr);
              return res.status(500).json({ error: 'Failed to create delivery record' });
            }
            
            console.log(`✅ Delivery created with ID ${this.lastID}`);
...
// Add delivery scan with package support
app.post('/api/deliveries/:deliveryId/scan', requireAuth, (req, res) => {
  const { deliveryId } = req.params;
  const { order_item_id, product_id, package_id, barcode, scanned_quantity, delivery_type } = req.body;
  
  console.log(`📱 Adding delivery scan for delivery ${deliveryId} - Type: ${delivery_type || 'simple'}`);
  
  // First check if delivery_scans table has package_id column
  const checkColumnQuery = `PRAGMA table_info(delivery_scans)`;
  db.all(checkColumnQuery, (err, columns) => {
    if (err) {
      console.error('❌ Error checking delivery_scans table:', err);
      return res.status(500).json({ error: 'Database error' });
    }
    
    const hasPackageId = columns.some(col => col.name === 'package_id');
    const hasDeliveryType = columns.some(col => col.name === 'delivery_type');
    
    // Add columns if they don't exist
    const alterQueries = [];
    if (!hasPackageId) {
      alterQueries.push(`ALTER TABLE delivery_scans ADD COLUMN package_id INTEGER DEFAULT NULL`);
    }
    if (!hasDeliveryType) {
      alterQueries.push(`ALTER TABLE delivery_scans ADD COLUMN delivery_type TEXT DEFAULT 'simple'`);
    }
    
    // Execute alter queries if needed
    const executeAlters = (queries, callback) => {
      ...
      db.run(query, (err) => {
        ...
        console.log('✅ Column added to delivery_scans table');
        executeAlters(queries, callback);
      });
    };
    
    executeAlters([...alterQueries], (alterErr) => {
      if (alterErr) {
        return res.status(500).json({ error: 'Failed to update table structure' });
      }
      
      // Insert the delivery scan
      const query = `
        INSERT INTO delivery_scans (
          delivery_id, order_item_id, product_id, package_id, 
          barcode, scanned_quantity, delivery_type, scan_status, scanned_by, scanned_at
        )
      ...
      db.run(query, [
        ...
        if (err) {
          ...
          return res.status(500).json({ error: 'Failed to add delivery scan' });
        }
        
        console.log(`✅ Delivery scan added with ID ${this.lastID} - Package: ${package_id || 'none'}`);
        res.json({ success: true, scan_id: this.lastID });
      });
    });
  });
});
...
// Complete delivery
app.post('/api/deliveries/:deliveryId/complete', requireAuth, (req, res) => {
  const { deliveryId } = req.params;
  const { customer_signature, delivery_notes, delivery_scans } = req.body;
  ...
  db.serialize(() => {
    ...
    db.run(updateQuery, [customer_signature, delivery_notes, deliveryId], function(err) {
      if (err) {
      ...
      db.get('SELECT order_id FROM deliveries WHERE id = ?', [deliveryId], (orderErr, delivery) => {
        if (orderErr) {
        ...
        db.run(
          ...
          (updateOrderErr) => {
            if (updateOrderErr) {
              console.error('❌ Order delivery status update error:', updateOrderErr);
              db.run('ROLLBACK');
              return res.status(500).json({ error: 'Failed to update order delivery status' });
            }
...
// Get delivery records for an order
app.get('/api/orders/:orderId/delivery-records', requireAuth, (req, res) => {
  const { orderId } = req.params;
  ...
  db.get(deliveryQuery, [orderId], (err, delivery) => {
    if (err) {
    ...
    if (!delivery) {
      return res.json({ delivery: null, scans: [] });
    }
    
    // Get delivery scans with product info, grouped by product
    const scansQuery = `
      SELECT 
        oi.sku, 
        p.name as product_name,
        p.color as product_color,
        COUNT(*) as total_packages_scanned,
        MIN(ds.scanned_at) as first_scan_time,
        MAX(ds.scanned_at) as last_scan_time,
        GROUP_CONCAT(DISTINCT pp.package_name) as package_names,
        SUM(ds.scanned_quantity) as total_quantity
      FROM delivery_scans ds
      LEFT JOIN order_items oi ON ds.order_item_id = oi.id
      LEFT JOIN products p ON oi.product_id = p.id
      LEFT JOIN product_packages pp ON ds.package_id = pp.id
      WHERE ds.delivery_id = ?
      GROUP BY oi.id, oi.sku, p.name, p.color
      ORDER BY MIN(ds.scanned_at) ASC
    `;
    
    db.all(scansQuery, [delivery.id], (scanErr, scans) => {
      if (scanErr) {
        console.error('❌ Delivery scans fetch error:', scanErr);
        return res.status(500).json({ error: 'Failed to fetch delivery scans' });
      }
      
      console.log(`📦 Found ${scans.length} unique products with delivery scans for delivery ${delivery.id}`);
...
app.get('/api/deliveries/:deliveryId', requireAuth, (req, res) => {
  const { deliveryId } = req.params;
  
  const query = `
    SELECT d.*, o.order_number, o.customer_name, o.fulfillment_status
    FROM deliveries d
    LEFT JOIN orders o ON o.id = d.order_id
    WHERE d.id = ?
  `;
  
  db.get(query, [deliveryId], (err, delivery) => {
    if (err) {
      console.error('❌ Delivery fetch error:', err);
      return res.status(500).json({ error: 'Failed to fetch delivery' });
    }
    ...
    // Get delivery scans
    const scansQuery = `
      SELECT ds.*, oi.sku, oi.product_name, oi.quantity as order_quantity
      FROM delivery_scans ds
      LEFT JOIN order_items oi ON oi.id = ds.order_item_id
      WHERE ds.delivery_id = ?
      ORDER BY ds.scanned_at DESC
    `;
    
    db.all(scansQuery, [deliveryId], (scansErr, scans) => {
      if (scansErr) {
        console.error('❌ Delivery scans fetch error:', scansErr);
        return res.status(500).json({ error: 'Failed to fetch delivery scans' });
      }
...
app.post('/api/inventory-counts', async (req, res) => {
  try {
    ...
    if (type === 'full') {
      // Full count - Include all products with shelf assignments
      items = await all(`
        SELECT 
          p.id as product_id,
          p.sku,
        ...
        LEFT JOIN shelf_packages sp ON pp.id = sp.package_id
        LEFT JOIN shelves s ON sp.shelf_id = s.id
        WHERE (s.shelf_code IS NOT NULL AND s.shelf_code != 'SSH-01-01')
        OR (s.shelf_code IS NULL AND pp.id IS NOT NULL)
        ORDER BY COALESCE(s.shelf_code, 'NO-LOCATION'), p.sku, COALESCE(pp.barcode, 'ZZZ')
      `);
    } else if (type === 'spot') {
...
app.get('/api/stock', async (req, res) => {
  try {
    ...
    if (hasShelfPackages) {
      ...
      query = `
        SELECT 
          ...
          CASE 
            WHEN COUNT(DISTINCT pp.id) > 0 THEN COUNT(DISTINCT sp.shelf_id)
            ELSE 0 
    ...
    // Location filtreleme - veritabanında yapılmalı
    if (location && location !== 'all') {
      // Location filtreleme için ayrı sorgu çalıştır
      const locationFilteredStocks = await all(`
        SELECT DISTINCT 
          p.id as product_id,
          p.sku,
          p.name as product_name
      ...
      const locationSkus = locationFilteredStocks.map(item => item.sku);
      stocks = stocks.filter(item => locationSkus.includes(item.sku));
    }
...
app.post('/api/orders/:orderId/convert-to-dispatch', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // 🚀 YENİ NETSIS REST API ENTEGRASYONİ
    console.log('🚀 Using Netsis REST API integration...');
    const deliveryNoteResult = await netsisRestAPI.createDispatchNote(order, orderItems);
    
    if (deliveryNoteResult.success) {
      console.log(`✅ Netsis REST API dispatch created: ${order.order_number} -> ${deliveryNoteResult.dispatchId}`);
...
app.get('*', (req, res) => {
  ...
  // Add delivery_status column for delivery verification system
  try {
    await run(`ALTER TABLE orders ADD COLUMN delivery_status TEXT DEFAULT NULL`);
    console.log('✅ Delivery status column added to orders table');
  } catch (e) {
    if (!e.message.includes('duplicate column name')) {
      console.log('ℹ️ Delivery status column already exists in orders table');
    }
  }
