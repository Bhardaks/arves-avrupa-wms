...
(function() {
    ...
    // Generate Code128 barcode with new format: 1 Letter + YYMMDDHHMMSS (13 characters)
    window.generateBarcode = function() {
        const productName = document.getElementById('pkgProductName')?.value?.trim();
        if (!productName) {
            showMessage('Önce ürün adını girin', 'error');
            return;
        }
        ...
        // Extract first letter from product name
        const prefix = productName.substring(0, 1).toUpperCase();
        
        // Generate YYMMDDHHMMSS format (12 digits)
        const year = date.getFullYear().toString().slice(-2);        // YY
        const month = (date.getMonth() + 1).toString().padStart(2, '0');  // MM
        const day = date.getDate().toString().padStart(2, '0');      // DD
        const hour = date.getHours().toString().padStart(2, '0');    // HH
        const minute = date.getMinutes().toString().padStart(2, '0'); // MM
        const second = date.getSeconds().toString().padStart(2, '0'); // SS
        
        const dateTime = year + month + day + hour + minute + second;
        
        // Combine: 1 letter + 12 digits = 13 characters total
        const barcode = prefix + dateTime;
        
        document.getElementById('pkgBarcode').value = barcode;
        showMessage(`Barkod otomatik oluşturuldu: ${barcode} (${productName.charAt(0).toUpperCase()} + ${dateTime})`, 'success');
    };
