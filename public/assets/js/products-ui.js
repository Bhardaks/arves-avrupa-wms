...
(function() {
    ...
    function createProductRow(product) {
        ...
        const duplicateWarning = getDuplicateBarcodeWarning(product);

        return `
            <tr data-product-id="${product.id}" ${duplicateWarning.hasWarning ? 'style="background-color: #ff9999;"' : ''}>
                <td>
    ...
    // Get duplicate barcode warning HTML
    function getDuplicateBarcodeWarning(product) {
        if (!product.packages || product.packages.length === 0) {
            return { hasWarning: false, html: '' };
        }
        ...
        for (const pkg of product.packages) {
            ...
            // Find other products with same barcode
            const otherProducts = window.products.filter(p => 
                p.id !== product.id && 
                p.packages && 
                p.packages.some(pkg2 => pkg2.barcode === pkg.barcode)
            );
        ...
        const warningHtml = duplicateBarcodes.map(dup => {
            const otherSkus = dup.otherProducts.map(p => p.sku).join(', ');
            return `<div class="duplicate-warning" style="color: #856404; font-size: 0.85em; margin-top: 4px;">
                🚨 Barkod çakışması: ${dup.barcode} → ${otherSkus}
            </div>`;
        }).join('');
        
        return { hasWarning: true, html: warningHtml };
    }
